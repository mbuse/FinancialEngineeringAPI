 ...
 * @author mbuse
 */
public class Caplet implements Security {

  private LatticeConfiguration lattice;
  private Double strike;
  private int maturity;

  public Caplet(LatticeConfiguration lattice, Double strike, int maturity) {
  ...
  public Double getValue(int t, int u) {
    ...
    if (t>periodOfSettlement) {
      ...
      return Math.max(0.0, (rate - strike))/(1 + rate);
    }
    else {
  ...
  public String toString() {
    return "Caplet (maturity: " + maturity
            + ", strike: " + strike
            + ", lattice: " + lattice + ")";
  }
